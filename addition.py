def create_db(database_name):#function to create database when the program is run for the first time
    import mysql.connector
    db_obj = mysql.connector.connect(host = "localhost", user = "root", password = "Mathew")
    cur_obj = db_obj.cursor()
    q = "craete database shop"
    try:
        cur_obj.execute(q)
...
def create_table():# function to create table stock when the program is run for the first time
    import mysql.connector
...
def create_table_billing():#function to create table bill when the program is run for the first time
    ...
    db_obj = mysql.connector.connect(host = "localhost", user = "root", password = "Mathew", database = 'shop')
    cur_obj = db_obj.cursor()
    q = "create table bill(sn_no int primary key,name varchar(20) not null,quantity int not null,iteam_num int foreign key references stock(iteam_num))"
    try:
        cur_obj.execute(q)
    except Exception:
        pass
...
def add_data_billing(sn_no,quantity,name,it_no):#fn to add data into the table bill
    import mysql.connector
    db_obj = mysql.connector.connect(host = "localhost", user = "root", password = "Mathew", database = 'shop')
    cur_obj = db_obj.cursor()
    q = f"insert into bill(sn_no,quantity,iteam_num) values({sn_no},{name},{quantity},{it_no})"
    try:
        cur_obj.execute(q)
        db_obj.commit()
    except Exception as e:
